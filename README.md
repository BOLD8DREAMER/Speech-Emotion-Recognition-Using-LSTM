...
This project aims to predict human emotions using LSTM (Long Short Term Memory). It is an artificial Recurrent Neural Network architecture used in the field of deep learning. An LSTM has feedback connections. It can, not only process single data points (such as images) but also entire sequences of data (such as audio or video).
